...
- Banco de frases: dicionários `LISTEN`/`READ` em `engine.py`, chaveados por `nível -> categoria -> lista`. `get_content` usa uma fila embaralhada por `(nível, módulo, categoria)` (sem repetir até esgotar). **Para adicionar frases, edite esses dicionários.**
...
- **STT**: `SpeechRecognition` + `ffmpeg`. O Google Speech API é usado (gratuito, sem chave).
