...
The project (English JATEL / English Flow) is an English teaching app: a FastAPI backend that serves both the API and a static frontend SPA from a single server.

- **`backend/`** — FastAPI app (`main.py`), business logic (`engine.py`), data-driven content (`grammar.json`, `memhack.json`), user store, and serves `frontend/`
...
- **Content is data-driven** (edit files, not code):
  ...
  - Grammar (CEFR A1–C2): `backend/grammar.json` (`{"levels": [...], "grammar": {<level>: [{topic, structure, explanation, examples}]}}`), loaded at startup (falls back to embedded). Reload at runtime via `POST /api/admin/reload-grammar` (admin only).
